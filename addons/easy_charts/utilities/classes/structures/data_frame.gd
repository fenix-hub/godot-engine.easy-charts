...
func get_headers_names(indexes : PackedInt64Array) -> PackedStringArray:
	var headers : PackedStringArray = []
...
func get_icolumns(indexes : PackedInt64Array) -> Array:
	var values : Array = []
	for index in indexes:
...
func get_labels_names(indexes : PackedInt64Array) -> PackedStringArray:
	var headers : PackedStringArray = []
	for index in indexes:
...
func get_irows(indexes : PackedInt64Array) -> Array:
	var values : Array = []
	for index in indexes:
...
func _get(_property : StringName):
	...
	if ":" in _property:
		...
		if (property[0]).is_valid_int(): 
			if property[1] == "*":
				return get_icolumns(range(int(property[0]), headers.size()-1))
			else:
				return get_icolumns(range(int(property[0]), int(property[1]) +1))
		else:
			if property[1] == "*":
	...
	# ";" --> Rows 
		...
		var property : PackedStringArray = _property.split(";")
		if (property[0]).is_valid_int(): 
			return get_irows(range(int(property[0]), int(property[1]) + 1 ))
		else: 
